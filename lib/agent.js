...
import { UNIVERSAL_PREAMBLE, getActiveAdapter } from './adapters.js';
...
15. scratchpad_read()                        -- read the persistent scratchpad
16. scratchpad_write(text, mode?)            -- save state to the persistent scratchpad
...
SCRATCHPAD:
- You have a persistent scratchpad that survives across many steps. Use it to hold download IDs, file paths, items already processed, running plans, and intermediate results.
- Access the scratchpad by including \`scratchpad_read\` as a tool. Read its content carefully before acting.
- Write to it with \`scratchpad_write({text})\` (append) or \`scratchpad_write({text, mode: "replace"})\` to overwrite.
- It is injected before each tool call, so you can always see it.`;
...
export class Agent {
  constructor(task) {
    ...
    this._scratchpad = this.task.state.scratchpad || ''; // persisted compact memory
  }

  get scratchpad() { return this._scratchpad; }
  _setScratchpad(text) { this._scratchpad = text; this.task.state.scratchpad = text; }
  ...
  async runStep() {
    ...
    // Site adapters — inject per-site guidance on first step.
    const pageUrl = snap.url || (state.pageContext && state.pageContext.url) || '';
    let adapterInjected = false;
    if (state.stepsTaken <= 1 && pageUrl) {
      const adapter = getActiveAdapter(pageUrl);
      if (adapter) {
        messages.push({ role: 'system', content: '[Site guidance for ' + adapter.name + ']\n' + adapter.notes });
        adapterInjected = true;
      }
    }
    if (state.stepsTaken <= 1 && !adapterInjected) {
      messages.push({ role: 'system', content: UNIVERSAL_PREAMBLE });
    }
    ...
    // Scratchpad — always inject it if populated so the model remembers state.
    if (this._scratchpad) {
      messages.push({ role: 'system', content: '[SCRATCHPAD — saved state]\n' + this._scratchpad });
    }
  ...
  async executeTool() {
    ...
    // _dispatchTool may have updated this.task.tabId (open_tab / switch_tab)
    const postTabId = this.task.tabId;
    const navTools = ['navigate', 'open_tab', 'back', 'refresh', 'switch_tab'];
    if (execResult && execResult.ok && navTools.includes(call.tool)) {
      try {
        await this._waitForTabLoad(postTabId, 8000);
        await new Promise(r => setTimeout(r, 2000));
        const fresh = await this._freshSnapshot(postTabId);
    ...
    // Unintended navigation detection — check if a non-navigation click changed the URL.
    const isNavTool = ['navigate', 'open_tab', 'back', 'refresh', 'switch_tab'].includes(call.tool);
    if (execResult && execResult.ok && (call.tool === 'click') && execResult.snapshot && state.pageContext) {
      const prevUrl = state.pageContext.url;
      const newUrl = execResult.snapshot.url;
      if (prevUrl && newUrl && newUrl !== prevUrl) {
        execResult._navWarning = '[NAVIGATION OCCURRED] The click on "' + (call.args && call.args.id) + '" unexpectedly changed the page URL from ' + (prevUrl.length > 60 ? prevUrl.slice(0, 60) + '...' : prevUrl) + ' to ' + (newUrl.length > 60 ? newUrl.slice(0, 60) + '...' : newUrl) + '. Re-plan from the new snapshot.';
      }
    }

    // Reactive tool injection — add context-sensitive notes to the tool result.
    let reactiveNote = '';
    if (execResult && execResult.ok) {
      if (call.tool === 'click') {
        if (!execResult._navWarning) {
          const dupClick = this._checkDuplicateClick(call);
          if (dupClick) reactiveNote = '[DUPLICATE CLICK] Similar click already fired within 45s. Verify the first click took effect before retrying.';
        }
      }
    }

    // Attach enriched result with nav warning / reactive note.
    if ((execResult && execResult._navWarning) || reactiveNote) {
      let enriched = JSON.stringify(execResult).replace(/\}$/, '');
      if (execResult._navWarning) {
        enriched += ',"_navWarning":"' + JSON.stringify(execResult._navWarning).slice(1, -1) + '"';
      }
      if (reactiveNote) {
        enriched += ',"_note":"' + JSON.stringify(reactiveNote).slice(1, -1) + '"';
      }
      enriched += '}';
      state.messages.push({ role: 'user', content: toolResultWrap(enriched) });
    } else {
    ...
    const maxSteps = this.task.maxSteps != null ? this.task.maxSteps : 25;
  ...
  _notify(title, message) {
    try {
      if (typeof chrome !== 'undefined' && chrome.notifications) {
        chrome.notifications.create(this.task.id + '_' + Date.now(), {
          type: 'basic',
  ...
  _checkDuplicateClick(call) {
    if (!call || !call.args || !call.args.id) return false;
    const now = Date.now();
    if (!this._clickTimestamps) this._clickTimestamps = {};
    const key = call.tool + ':' + call.args.id;
    const last = this._clickTimestamps[key] || 0;
    this._clickTimestamps[key] = now;
    return (now - last) < 45000; // same click within 45s
  }
  ...
  _recordAction(call) {
    ...
    if (settings.enabled !== false) {
      // Graduated response: nudge first, abort after repeated violations
      if (sameCount >= (settings.sameActionLimit || 3)) {
        if (sameCount >= 8 || this._loopAborted) {
          this._abort({ kind: 'error', message: 'loop_detected: same action ' + summary + ' repeated ' + sameCount + ' times' });
          this._loopAborted = true;
        } else if (sameCount >= (settings.sameActionLimit || 3)) {
          // Soft nudge — inject warning into context
          const state = this.task.state;
          state.messages.push({ role: 'user', content: '[LOOP WARNING] ' + summary + ' has been repeated ' + sameCount + ' times. Do something different or explain your strategy. Two different healthy calls will reset this counter.' });
        }
      } else {
        this._loopNudgeCount = 0; // different action resets
      }
    }
    ...
    // Coordinate-click loop: bucket clicks by args.id (round to nearest 5 via suffix)
    if (call.tool === 'click' && call.args && call.args.id) {
      if (!this._coordClickCounts) this._coordClickCounts = {};
      const coord = call.args.id; // element IDs may indicate stale slots
      this._coordClickCounts[coord] = (this._coordClickCounts[coord] || 0) + 1;
      const cCount = this._coordClickCounts[coord];
      if (cCount >= 5) {
        const state = this.task.state;
        state.messages.push({ role: 'user', content: '[COORD CLICK LOOP] Same element "' + coord + '" clicked ' + cCount + ' times. Take a fresh snapshot and try a different approach.' });
      }
      if (cCount >= 8) {
        this._abort({ kind: 'error', message: 'coordinate_click_loop: element "' + coord + '" clicked ' + cCount + ' times' });
        return;
      }
    }
  ...
  async _dispatchTool(tabId, call, snapshot) {
    if (call.tool === 'finish') {
      // Pre-done check: verify scratchpad tasks are resolved.
      let preCheck = '';
      if (this._scratchpad) {
        const unresolved = this._scratchpad.match(/pending|TODO|in-progress/gi);
        if (unresolved && unresolved.length > 0) {
          preCheck = '[BLOCKED DONE] Scratchpad has ' + unresolved.length + ' unresolved items. Complete them or call scratchpad_write to mark done before finishing.';
          return { ok: false, blockedDone: true, error: preCheck };
        }
      }
    ...
    if (call.tool === 'scratchpad_read') {
      return { ok: true, content: this._scratchpad || '(empty)' };
    }
    if (call.tool === 'scratchpad_write') {
      const text = (call.args && call.args.text) || '';
      if (!text) return { ok: false, error: 'no_text' };
      const mode = (call.args && call.args.mode) || 'append';
      if (mode === 'replace') {
        this._setScratchpad(text);
      } else {
        this._setScratchpad((this._scratchpad ? this._scratchpad + '\n' : '') + text);
      }
      await this._persistState();
      return { ok: true, content: this._scratchpad };
    }
  ...
  async _done(answer) {
    ...
    this._notify('Task Complete', answer.slice(0, 200) || 'Task finished successfully.');
  }
  ...
  async _abort(err) {
    ...
    const isError = this.task.state.status === 'error';
    this._notify(isError ? 'Task Failed' : 'Task Stopped', (err && err.message) || (isError ? 'An error occurred during task execution.' : 'The task was stopped.'));
  }
